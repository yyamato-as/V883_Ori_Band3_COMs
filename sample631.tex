...
As the disk emission is only marginally spatially resolved, it is difficult to directly infer the distribution of emission in the inner region, particularly the cavity region seen in sub-mm observations \citep{vantHoff2018, Lee2019, Tobin2023}. Alternatively, we can indirectly probe the emission in the inner region which is not spatially resolved in the velocity-integrated intensity maps from the line profile assuming gas kinematics. If we assume that the observed gas follows the Keplerian rotation, the high-velocity components with respect to the systemic velocity contains the information about the emission from the inner region. The maximum velocity of Keplerian rotation as a function of disk radius ($v_\mathrm{max}(r)$) can be written as 
\begin{equation}
    v_\mathrm{max}(r) = \sqrt{\frac{GM_\star}{r}}\sin i,
\end{equation}
where $G$ is the gravitational constant, $M_\star$ is the central stellar mass, and $i$ is the inclination of the disk. Using this relation, the radial intensity profile (as a function of radius) can be reconstructed from the line profile (as a function of velocity) as demonstrated by \citet{Bosman2021}. Based on the forward modeling described in \citet{Bosman2021}, we employed a similar approach to construct the radial intensity profiles of three bright transitions of \methanol, \acetaldehyde, and \methylformate from their line profile. The details of fitting can be found in Appendix \ref{}.
