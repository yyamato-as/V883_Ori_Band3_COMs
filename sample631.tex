...
We employed the Affine invariant Markov Chain Monte Carlo (MCMC) algorithm implemented in the \texttt{emcee} package \citep{emcee} to explore the parameter spaces. Two types of fits were conducted, one of which assumed a fixed emitting radius of 0\farcs6 based on the COM emission extent in higher-resolution Band 6 observations \citep{Lee2019, Tobin2023}, and another of which left it as a free parameter. For both fits, we run 400 walkers for 5000 steps, and the initial 4000 steps are discarded as burn-in.

Figure \ref{} demonstrates the unique capability of the simultaneous spectral fit to fully utilize the transitions even with severe spectral blending. 
